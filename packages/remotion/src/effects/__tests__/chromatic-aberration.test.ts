...
describe('ChromaticAberration schema', () => {
  ...
  it('rejects intensity out of range', () => {
    expect(chromaticAberrationSchema.safeParse({
      type: 'chromatic-aberration', startTime: 0, endTime: 30, intensity: 0.5,
    }).success).toBe(false);
  });

  it('is included in discriminated union', () => {
    const result = effectSegmentSchema.safeParse({
      type: 'chromatic-aberration', startTime: 0, endTime: 30,
    });
    expect(result.success).toBe(true);
  });
});
