...
describe('ProgressRing schema', () => {
  ...
  it('rejects targetPercent out of range', () => {
    expect(progressRingSchema.safeParse({
      type: 'progress-ring', startTime: 0, endTime: 3, targetPercent: 150,
    }).success).toBe(false);
  });

  it('requires targetPercent', () => {
    expect(progressRingSchema.safeParse({
      type: 'progress-ring', startTime: 0, endTime: 3,
    }).success).toBe(false);
  });

  it('is included in discriminated union', () => {
    const result = effectSegmentSchema.safeParse({
      type: 'progress-ring', startTime: 0, endTime: 3, targetPercent: 50,
    });
    expect(result.success).toBe(true);
  });
});
