...
// ── Test 2: Webinar Promo (various webinar templates, techskills brand) ─
console.log('\n3. Test Reel #2: "Webinar Promo" (webinar templates + techskills brand)');
const test2Slides = [
  ...
  { template: 'webinar-cta-slide', title: 'Register Now', text: 'March 25, 2026 at 7 PM', cta: 'techskills.academy/webinar', badge: 'FREE', num: '4' },
];
...
for (let i = 0; i < test2Slides.length; i++) {
  ...
  const bytes = await renderToFile(
    { brand: 'techskills', template: tmpl, size: 'story', title: slide.title, text: slide.text, badge: slide.badge, num: slide.num, cta: slide.cta ?? '' },
    outPath,
  );
  console.log(`   ✓ Slide ${i + 1} (${tmpl}): ${outPath} (${(bytes / 1024).toFixed(0)} KB)`);
}

// ── Test 3: Quote series (quote-card template, fundacja brand) ─
console.log('\n4. Test Reel #3: "Motivational Quotes" (quote-card + fundacja brand)');
const test3Slides = [
  { title: '', text: 'Jedyną stałą w życiu jest zmiana.', attr: '— Heraklit', badge: '#1', num: '1' },
  { title: '', text: 'Nie musisz być wspaniały, żeby zacząć. Musisz zacząć, żeby być wspaniały.', attr: '— Zig Ziglar', badge: '#2', num: '2' },
  { title: '', text: 'Najlepszy czas na posadzenie drzewa był 20 lat temu. Drugi najlepszy czas jest teraz.', attr: '— Przysłowie chińskie', badge: '#3', num: '3' },
];
...
for (let i = 0; i < test3Slides.length; i++) {
  ...
  const bytes = await renderToFile(
    { brand: 'fundacja', template: 'quote-card', size: 'story', text: slide.text, attr: (slide as Record<string, string>).attr ?? '', badge: slide.badge, num: slide.num },
    outPath,
  );
  console.log(`   ✓ Slide ${i + 1}: ${outPath} (${(bytes / 1024).toFixed(0)} KB)`);
}
