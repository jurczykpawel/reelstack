...
const tests: ReelTest[] = [
  ...
  },
  {
    ...
    template: 'quote-card',
    slides: [
      { title: '', text: 'Jedyną stałą w życiu jest zmiana.', badge: '#1', num: '1' },
      { title: '', text: 'Nie musisz być wspaniały, żeby zacząć.', badge: '#2', num: '2' },
      { title: '', text: 'Najlepszy czas jest teraz.', badge: '#3', num: '3' },
    ],
    narration: 'Jedyną stałą w życiu jest zmiana. Nie musisz być wspaniały, żeby zacząć. Musisz zacząć, żeby być wspaniały. Najlepszy czas na posadzenie drzewa był dwadzieścia lat temu. Drugi najlepszy czas jest teraz.',
  },
];
...
for (const test of tests) {
  ...
  const language = test.name === 'cytaty-pl' ? 'pl-PL' : 'en-US';
