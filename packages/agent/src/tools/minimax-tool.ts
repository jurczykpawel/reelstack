...
- video-01: smoother, better for calm/atmospheric shots`;
  ...
  async generate(request: AssetGenerationRequest): Promise<AssetGenerationJob> {
    if (!this.apiKey) {
      return { jobId: randomUUID(), toolId: this.id, status: 'failed', error: 'MINIMAX_API_KEY not set' };
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        log.warn({ status: res.status, errorPreview: errBody.substring(0, 200) }, 'MiniMax generate failed');
        return { jobId: randomUUID(), toolId: this.id, status: 'failed', error: `MiniMax API error (${res.status})` };
      }

      const data = (await res.json()) as { task_id?: string; base_resp?: { status_code: number; status_msg: string } };

      if (data.base_resp?.status_code !== 0) {
        return { jobId: randomUUID(), toolId: this.id, status: 'failed', error: data.base_resp?.status_msg ?? 'MiniMax error' };
      }
      ...
      return { jobId: randomUUID(), toolId: this.id, status: 'failed', error: `MiniMax request failed: ${err instanceof Error ? err.message : 'unknown'}` };
    }
  }
  ...
  async poll(jobId: string): Promise<AssetGenerationStatus> {
    ...
    try {
      const res = await fetch(`${MINIMAX_BASE}/query/video_generation?task_id=${encodeURIComponent(jobId)}`, {
      ...
      if (data.status === 'Fail') {
        return { jobId, toolId: this.id, status: 'failed', error: data.base_resp?.status_msg ?? 'MiniMax generation failed' };
      }
      ...
      const fileRes = await fetch(`${MINIMAX_BASE}/files/retrieve?file_id=${encodeURIComponent(data.file_id)}`, {
        headers: { Authorization: `Bearer ${this.apiKey}` },
        signal: AbortSignal.timeout(10_000),
      });

      if (!fileRes.ok) {
        log.warn({ jobId, fileId: data.file_id, status: fileRes.status }, 'MiniMax file retrieve failed');
        return { jobId, toolId: this.id, status: 'failed', error: `MiniMax file retrieve error (${fileRes.status})` };
      }
      ...
      if (!url) {
        return { jobId, toolId: this.id, status: 'failed', error: 'No download_url in MiniMax file response' };
      }
